...
import { IonModal, IonList, IonItem, IonButton, IonContent, IonToolbar, IonTitle, IonIcon } from '@ionic/react';
...
const EstadoPistas: React.FC = () => {
  ...
  useEffect(() => {
    const fetchClubs = async () => {
      ...
      try {
        ...
        setClubes(clubs);
        // Si hay club guardado, seleccionarlo
  ...
  // Quita el foco de cualquier elemento activo antes de abrir el modal
  ...
  return (
    <>
      <IonModal isOpen={showModal} className="modal-club-ionic" onDidDismiss={handleCloseModal} backdropDismiss={true}>
        <IonContent className="modal-club-content">
          <div className="modal-club-toolbar">
            <IonToolbar>
              ...
              <IonButton className="cerrar-modal-btn-ion" fill="clear" shape="round" onClick={handleCloseModal} slot="end">
          ...
          {loading ? (
            ...
            <IonList className="modal-club-list" style={{maxHeight: clubes.length > 7 ? 320 : 'none'}}>
              {clubes.map((club) => (
                <IonItem key={club.id} lines="none" className="modal-club-item">
                  <IonButton className="club-option-btn-modern" expand="block" fill="solid" onClick={() => handleSelectClub(club)}>
                    <IonIcon icon={businessSharp} style={{marginRight:8, fontSize:'1.2em'}} />
                    {club.nombre}
                  </IonButton>
                </IonItem>
              ))}
        ...
        <div className="estado-pistas-grid-real">
          {pistas.map((pista, idx) => (
            <div className="estado-pista-card" key={idx}>
              <p>{pista.nombre}</p>
              <span className={
                pista.estado === 'Disponible' ? 'disponible' : pista.estado === 'Ocupada' ? 'ocupada' : 'mantenimiento'
              }>
                {pista.estado}
              </span>
            </div>
